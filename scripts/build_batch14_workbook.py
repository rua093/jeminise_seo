...
PREVIOUS = BATCHES_DIR / "SEO_Product_Optimization_through_batch_013.xlsx"
OUTPUT = BATCHES_DIR / "SEO_Product_Optimization_through_batch_014.xlsx"


WOLF_REFS = "https://www.ebay.com/b/Wolf-Quilt-In-Duvet-Covers-Bedding-Sets/37644/bn_7022238087; https://www.wayfair.com/keyword.php?keyword=wolf+bedding; https://www.amazon.com/wolf-comforter-set/s?k=wolf+comforter+set"
SOFTBALL_REFS = "https://www.amazon.com/Beautiful-Softball-Comforter-Blankets-Lightweight/dp/B0DC6QXB1T; https://www.etsy.com/listing/556448324/girls-softball-bedding-for-girls; https://www.youcustomizeit.com/p/Softball-Comforters-Personalized/353832"
CACTUS_REFS = "https://www.amazon.com/cactus-quilt-set/s?k=cactus+quilt+set; https://www.wayfair.com/keyword.php?keyword=cactus+bedding; https://www.etsy.com/market/king_cactus_quilt_set"
DRAGONFLY_REFS = "https://www.etsy.com/; https://www.walmart.com/; https://www.pinterest.com/; https://seenorest.com/"
ELEPHANT_REFS = "https://www.amazon.com/elephant-quilts-queen-size/s?k=elephant+quilts+queen+size; https://ohaprints.com/products/ohaprints-quilt-bed-set-pillowcase-cute-elephant-frame-patchwork-floral-pattern-unique-gift-for-animal-lover-blanket-bedspread-bedding-241; https://www.etsy.com/market/indian_patchwork_elephant_quilt"
DRAGON_REFS = "https://www.amazon.com/Dragon-Quilt-Set-Bedding-Comforter/dp/B0DJFG276X; https://www.pinterest.com/pin/turquoise-dragon-scales-pattern-print-duvet-cover-bedding-set--143059725653473167/; https://www.wayfair.com/; https://www.temu.com/dragon-bedding-set-5030175152007-s.html"


PROFILES = {
    131: ("wolf wearing headdress close up quilt", "wolf headdress quilt", "Wolf Headdress Quilt Set", "Wolf Wearing Headdress Quilt Set", "close-up wolf head wearing a headdress-inspired design on a tan quilt with matching shams", "wolf bedding, Native inspired wolf quilt, wolf head quilt set", "EVERGREEN", WOLF_REFS),
    132: ("purple and yellow softball comforter with Olivia name", "softball comforter with name", "Softball Comforter with Name", "Purple Yellow Softball Comforter with Name", "bright purple and yellow softball artwork with Olivia name and matching softball-player pillowcases", "girls softball bedding, personalized softball comforter, softball bedding set", "EVERGREEN", SOFTBALL_REFS),
    133: ("desert cactus flower patchwork quilt", "cactus quilt set", "Cactus Quilt Set", "Desert Cactus Flower Patchwork Quilt", "desert cactus and flower patchwork artwork with potted cacti, daisies and green quilt blocks", "desert bedding, cactus bedding set, Southwestern quilt set", "EVERGREEN", CACTUS_REFS),
    134: ("purple floral dragonfly stained glass quilt", "dragonfly stained glass quilt", "Dragonfly Stained Glass Quilt", "Purple Floral Dragonfly Stained Glass Quilt", "purple floral stained-glass style artwork with multiple dragonflies and matching shams", "dragonfly bedding, floral dragonfly quilt, stained glass quilt set", "EVERGREEN", DRAGONFLY_REFS),
    135: ("yellow pond dragonfly patchwork quilt", "dragonfly patchwork quilt", "Dragonfly Patchwork Quilt Set", "Yellow Pond Dragonfly Patchwork Quilt", "large dragonfly above a yellow water lily and pond grass in a blue and gold patchwork frame", "dragonfly bedding, water lily quilt, nature quilt set", "EVERGREEN", DRAGONFLY_REFS),
    136: ("blue yellow dragonfly pond quilt", "dragonfly pond quilt", "Dragonfly Pond Quilt Set", "Blue Yellow Dragonfly Pond Quilt", "dragonfly over pond water, yellow lilies and green reeds in a stained-glass circular frame", "dragonfly bedding, pond quilt set, floral dragonfly quilt", "EVERGREEN", DRAGONFLY_REFS),
    137: ("purple vintage dragonfly quilt set", "purple dragonfly quilt", "Purple Dragonfly Quilt Set", "Purple Vintage Dragonfly Quilt Set", "large colorful dragonfly with flowers on a purple stained-glass patchwork background", "dragonfly bedding, vintage dragonfly quilt, floral quilt set", "EVERGREEN", DRAGONFLY_REFS),
    138: ("colorful elephant patchwork quilt on pale blue background", "elephant patchwork quilt", "Elephant Patchwork Quilt Set", "Colorful Elephant Patchwork Quilt", "colorful patchwork elephant on a pale blue quilt with multicolor border and matching shams", "elephant bedding, animal farmhouse quilt, boho elephant quilt", "EVERGREEN", ELEPHANT_REFS),
    139: ("navy yellow elephant floral patchwork quilt", "yellow elephant quilt", "Yellow Elephant Quilt Set", "Navy Yellow Elephant Patchwork Quilt", "yellow and white elephant patchwork artwork on navy background with floral accents and matching shams", "elephant bedding, patchwork elephant quilt, animal quilt bedset", "EVERGREEN", ELEPHANT_REFS),
    140: ("rainbow fantasy dragon quilt set", "fantasy dragon quilt", "Fantasy Dragon Quilt Set", "Rainbow Fantasy Dragon Quilt Set", "large rainbow fantasy dragon head with blue, purple, orange and gold scales on a quilt set", "dragon bedding, dragon scale quilt, fantasy bedding set", "EVERGREEN", DRAGON_REFS),
}
...
def main():
    ...
    for summary in summaries:
        ...
        if pos == 131:
            issue_bits.append("Wolf headdress-inspired wording should be reviewed for brand suitability and cultural sensitivity before deployment.")
        if 134 <= pos <= 137:
            issue_bits.append("Closely related dragonfly quilt variants should be reviewed for cannibalization and motif-specific title differentiation.")
        if pos in (138, 139):
            issue_bits.append("Elephant patchwork variants are close; color and background differences should remain explicit in titles and alt text.")
        if html_error:
        ...
        desc = (
            f"<p>This Jeminise {kind} features {detail}, making it suited for themed decor, sports rooms, garden-inspired spaces or fantasy bedrooms.</p>"
        ...
        append_dict(wb["Buyer_Search_Research"], {
            ...
            "jtbd_statement": f"When shopping for themed bedding, the buyer wants a {kind} matching a specific sport, animal, floral, desert or fantasy motif.",
            "functional_motivation": "Find the right design, size, bedding type, care details and decor fit.",
            "emotional_social_motivation": "Create a distinctive bedroom look or give a themed sports, garden, wildlife or fantasy gift.",
    ...
    for row in [
        {"metric": "batch_014_products", "value": "10", "definition": "Products appended in this cumulative workbook"},
        {"metric": "batch_014_images", "value": str(added_images), "definition": "Gallery images downloaded and viewed for batch_014"},
        {"metric": "cumulative_products", "value": "140", "definition": "Total products included through batch_014"},
    ]:
