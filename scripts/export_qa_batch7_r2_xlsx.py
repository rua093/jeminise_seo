...
base.OUTPUT = ROOT / "resutls" / "jeminise.com" / "20260906_234129" / "qa" / RUN / "SEO_QA_qa_batch_007_r2.xlsx"
...
if __name__ == "__main__":
    ...
    audit = {
        "path": str(base.OUTPUT), "sheet_names": wb.sheetnames, "expected_sheet_names": list(base.SHEETS),
        ...
        "wrap_text": {ws.title: bool(ws.cell(2, 1).alignment.wrap_text) for ws in wb.worksheets},
        ...
        "source_sha256": hashlib.sha256((ROOT / "resutls" / "jeminise.com" / "20260906_234129" / "revisions" / "qa_batch_007_r2" / "SEO_Product_Optimization_qa_batch_007_r2.xlsx").read_bytes()).hexdigest(),
        "zip_bad_member": zipfile.ZipFile(base.OUTPUT).testzip(),
    }
    expected = {name: len(payload[name]) for name in base.SHEETS}
    audit["expected_row_counts"] = expected
    audit["passed"] = (audit["sheet_names"] == audit["expected_sheet_names"] and audit["row_counts"] == expected
        and all(audit["freeze_panes"].values()) and all(audit["auto_filters"].values())
        and all(audit["wrap_text"].values()) and audit["formula_error_tokens"] == 0 and audit["zip_bad_member"] is None)
    path = ROOT / "seo_runs" / "jeminise.com" / "20260906_234129" / "qa" / RUN / "spreadsheet_validation.json"
