...
QA_RUN_ID = "20260909_143812"
BATCH_ID = "qa_batch_030_r5"
SOURCE = ROOT / "resutls" / SHOP / RUN_ID / "revisions" / "qa_batch_030_r5" / "SEO_Product_Optimization_qa_batch_030_r5.xlsx"
...
WOLF_EVIDENCE_ID = "evidence_batch_030_295"

SERP_NOTES = {
    "reader": "Public SERP showed comparable personalized reading/book-lover blanket pages; no first-party volume.",
    "mom": "Public SERP showed comparable Best Mom Ever and photo-collage blanket/quilt gift pages; no first-party volume.",
    "wolf": "Public SERP showed comparable southwestern wolf bedding/quilt/comforter pages; no first-party volume.",
...
def product_group(handle):
    if "reading" in handle or "book" in handle:
        return "reader"
    if "mother" in handle or "mom" in handle:
        return "mom"
    if "wolf" in handle:
        return "wolf"
...
def main():
    ...
    for raw in ws.iter_rows(min_row=2, values_only=True):
        ...
        if str(row.get("evidence_id") or "").startswith("evidence_batch_030_"):
            products.append(row)
    ...
    for p in products:
        ...
        is_wolf = p.get("evidence_id") == WOLF_EVIDENCE_ID
        type_conflict = is_wolf and "comforter" in public_fields.lower()
        ...
        if type_conflict:
            ...
            issues.append({
                ...
                "source_observation": f"Admin Type={first.get('Type')}; live JSON type={l['json_type']}; options={l['options']}; contact sheet title says Quilt and size panel says 1 Premium Quilt.",
                "reason": "The proposed public SEO copy calls product 295 a comforter set while stronger product evidence supports Quilt/Quilt Set. This can mislead shoppers and makes sheet evidence inconsistent.",
                "recommended_fix": "Revise product 295 proposed fields, Keyword_Map, Product_Evidence, Buyer_Search_Research and 5 Image_Audit rows to use Quilt/Quilt Set unless merchant explicitly confirms comforter should be public product type.",
                "supporting_evidence": evrefs,
                "recheck_condition": "No comforter wording remains in proposed public fields or image alt text for product 295, except immutable current URL/source title if kept as source data.",
            })
            issue_no += 1
        ...
        if p.get("revision") not in {"r4", "r5"}:
            iid = f"ISSUE_{issue_no:03d}"
            issues.append({
                "issue_id": iid,
                "product_key": product_key,
                ...
                "source_observation": "Source workbook path is qa_batch_030_r5 but row-level revision remains from an older substantive revision.",
        ...
        for img in imgs:
            ...
            img_conflict = is_wolf and "comforter" in (obs + " " + alt).lower()
        ...
        criteria = [
            criterion(product_key, "P1", "FULL", "Visible artwork and product form match admin/export/live/contact sheet evidence; product 295 type wording conflict is scored in public-field criteria." if type_conflict else "Product type, artwork, color and design details match evidence.", evrefs, issue_refs[0] if type_conflict else ""),
            ...
            criterion(product_key, "D2", "PARTIAL" if type_conflict or template_leak else "FULL", "Body is specific but repeatedly calls product 295 a comforter/comforter set." if type_conflict else "Body HTML is complete, design-specific and free of internal/template wording.", evrefs, issue_refs[0] if type_conflict else ""),
            {"product_key": product_key, "criterion_id": "I1", "weight": 20, "assessment": "DERIVED", "rating": "PARTIAL" if image_avg < 100 else "FULL", "earned_points": i1_points, "assessed_weight": 20, "reason": "All images are present; product 295 alt text is partially downgraded for comforter/quilt product-type conflict." if image_avg < 100 else "All images are present; observations and alt text are specific, truthful and natural.", "evidence_refs": "Image_Audit; contact sheets; live JSON image count; admin export image count", "issue_refs": issue_refs[0] if image_avg < 100 else ""},
            criterion(product_key, "E1", "FULL" if p.get("revision") == "r5" else "PARTIAL", "Source can be traced; r5 row metadata matches the revised product." if p.get("revision") == "r5" else "Source can be traced, but row revision remains from an older substantive revision.", evrefs, "" if p.get("revision") == "r5" else issue_refs[-1]),
        ]
    ...
    xlsx = OUT_DIR / "SEO_QA_qa_batch_030_r5.xlsx"
    md = OUT_DIR / "SEO_QA_qa_batch_030_r5.md"
    ...
    md.write_text(f"""# SEO QA qa_batch_030_r5
