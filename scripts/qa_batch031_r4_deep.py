...
QA_RUN_ID = "20260909_135826"
BATCH_ID = "qa_batch_031_r4"
SOURCE = ROOT / "resutls" / SHOP / RUN_ID / "revisions" / "qa_batch_031_r4" / "SEO_Product_Optimization_qa_batch_031_r4.xlsx"
...
SERP_NOTES = {
    ...
    "deer": "Public SERP showed comparable deer quilt/comforter bedding pages from Amazon-style store pages, Target and wildlife bedding retailers; no first-party volume.",
}
...
def main():
    ...
    for p in products:
        ...
        public_fields = " ".join(str(p.get(c) or "") for c in ["primary_keyword", "secondary_keywords", "title_proposed", "meta_title_seo", "meta_description_seo", "description_proposed_html", "meta_keyword"])
        body = html_text(p.get("description_proposed_html"))
        meta = p.get("meta_description_seo") or ""
        is_310 = str(p.get("evidence_id")) == "evidence_batch_031_310"
        type_conflict = is_310 and "comforter" in public_fields.lower()
        ...
        if type_conflict:
            ...
            issues.append({
                ...
                "submitted_value": f"title/meta/body/keyword/alt use comforter; product_type={p.get('product_type')}",
                "source_observation": f"Admin Type={first.get('Type')}; live JSON type={l['json_type']}; options={l['options']}; live/admin SEO description uses quilt; contact sheet includes quilt/bedspread panels.",
                "reason": "The proposed public SEO fields keep comforter wording while the stronger product evidence supports Quilt/Quilt Set. This affects title, meta, body, keyword map and all image alt text for product 310.",
                "recommended_fix": "Revise product 310 proposed SEO fields, Keyword_Map, Product_Evidence and image alt text to use Quilt/Quilt Set unless merchant confirms comforter should be the public product type.",
                "supporting_evidence": evrefs,
                "recheck_condition": "No comforter wording remains in proposed fields/alt for product 310, or merchant confirmation is attached.",
            })
            issue_no += 1
        ...
        issues.append({
            ...
            "source_observation": "Source workbook path is qa_batch_031_r4 but row-level revision remains r2.",
        ...
        criteria = [
            criterion(product_key, "P1", "FULL", "Visible artwork, product form and design details match admin/export/live/contact sheet evidence; product 310 type wording conflict is scored in public-field criteria." if type_conflict else "Product type, artwork, color and design details match evidence.", evrefs, issue_refs[0] if type_conflict else ""),
            ...
            criterion(product_key, "K1", "PARTIAL" if type_conflict else "FULL", "Long-tail target fits the design but uses conflicting comforter product type." if type_conflict else "Long-tail target fits the visible product design and product-page intent.", f"{evrefs}; {SERP_NOTES[group]}", issue_refs[0] if type_conflict else ""),
            ...
            criterion(product_key, "D2", "PARTIAL" if type_conflict or template_leak else "FULL", "Body is specific but repeatedly calls product 310 a comforter/comforter set." if type_conflict else "Body HTML is complete, design-specific and free of internal/template wording.", evrefs, issue_refs[0] if type_conflict else ""),
            {"product_key": product_key, "criterion_id": "I1", "weight": 20, "assessment": "DERIVED", "rating": "PARTIAL" if image_avg < 100 else "FULL", "earned_points": i1_points, "assessed_weight": 20, "reason": "All images are present; product 310 alt text is partially downgraded for comforter/quilt product-type conflict." if image_avg < 100 else "All images are present; observations and alt text are specific, truthful and natural.", "evidence_refs": "Image_Audit; contact sheets; live JSON image count; admin export image count", "issue_refs": issue_refs[0] if image_avg < 100 else ""},
    ...
    xlsx = OUT_DIR / "SEO_QA_qa_batch_031_r4.xlsx"
    md = OUT_DIR / "SEO_QA_qa_batch_031_r4.md"
    ...
    md.write_text(f"""# SEO QA qa_batch_031_r4
