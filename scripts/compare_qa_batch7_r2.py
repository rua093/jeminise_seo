...
REVISION = ROOT / "resutls/jeminise.com/20260906_234129/revisions/qa_batch_007_r2/SEO_Product_Optimization_qa_batch_007_r2.xlsx"
OUTPUT = RUN_ROOT / "qa/20260907_164400/revision_diff.json"
...
inventory = json.loads((RUN_ROOT / "inventory.json").read_text(encoding="utf-8-sig"))
selected = inventory[60:70]
keys = {row["product_key"] for row in selected}
handles = {row["Handle"] for row in selected}
urls = {row["product_url"] for row in selected}
specs = {
...
for sheet, (include, identify) in specs.items():
    ...
    for row_id in sorted(set(before) | set(after)):
        ...
        if fields:
            changes.append({"row_id": row_id, "changes": fields})
