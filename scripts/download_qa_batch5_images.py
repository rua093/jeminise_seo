...
QA_DIR = ROOT / "seo_runs" / "jeminise.com" / "20260906_234129" / "qa" / "20260907_101050"
...
def main():
    ...
    for entry in live:
        ...
        for index, media in enumerate(entry["live"]["product_js"]["media"], 1):
            ...
            progress={"rubric_version":"1.0","qa_run_id":"20260907_101050","source_workbook":str(QA_DIR/"source_snapshot"/"SEO_Product_Optimization_through_batch_034.xlsx"),"batch_id":"qa_batch_005","batch_product_keys":[x["inventory"]["product_key"] for x in live],"current_product_key":product_key,"current_stage":"IMAGE_DOWNLOADED_PENDING_DIRECT_REVIEW","completed_image_keys":completed,"last_saved_at":datetime.now().astimezone().isoformat(),"awaiting_confirmation":False}
    ...
    print(f"downloaded={len(rows)} bytes={sum(x['bytes'] for x in rows)}")
