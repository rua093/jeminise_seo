...
module.QA_RUN_ID = "20260907_102634"
...
def main():
    with module.INVENTORY.open(newline="", encoding="utf-8-sig") as handle:
        batch = list(csv.DictReader(handle))[50:60]
    ...
    for row in batch:
        ...
        live.append(record)
        checkpoint = {"rubric_version":"1.0", "qa_run_id":module.QA_RUN_ID, "source_workbook":str(module.WORKBOOK), "batch_id":"qa_batch_006", "batch_product_keys":[item["product_key"] for item in batch], "current_product_key":row["product_key"], "current_stage":"LIVE_SOURCE_COLLECTED", "completed_image_keys":[], "awaiting_confirmation":False}
