...
BATCH_ID = "qa_batch_009_r3"
QA_RUN_ID = "20260907_180500"

SOURCE = ROOT / "resutls" / SHOP / RUN_ID / "revisions" / "qa_batch_009_r2" / "SEO_Product_Optimization_qa_batch_009_r2.xlsx"
...
OUTPUT = RESULT_DIR / "SEO_Product_Optimization_qa_batch_009_r3.xlsx"
REPORT = RESULT_DIR / "SEO_Product_Optimization_qa_batch_009_r3.md"
...
IMAGE_DETAILS = {
    ...
    82: {
        ...
        2: ("Holiday fireplace mockup with Daniel baseball batter blanket and promotional customization text visible in the image.", "Baseball batter blanket with Daniel in holiday mockup"),
        3: ("Outdoor autumn mockup of Daniel baseball batter stadium flag blanket.", "Daniel baseball batter blanket held outdoors"),
        4: ("Porch mockup of Daniel baseball batter blanket with promotional customization text visible in the image.", "Baseball batter flag blanket porch mockup"),
...
def customizer_sentence(audit):
    ...
    parts = []
    for label, required, min_length, max_length in matches:
    ...
    labels = " ".join(label for label, *_ in matches).lower()
    if "number" in labels:
...
def description(pos, admin_row, ctext):
    ...
    return (
        ...
        f"<li>Product name: {admin_row['Title']}.</li></ul>"
...
def main():
    ...
    for row_num in range(2, ws.max_row + 1):
        ...
        ws.cell(row_num, idx["issues"]).value = (
            f"R3 deep recheck after Sang QA: used products_export_1.csv (sha256:{admin_hash}) as admin baseline; "
    ...
    for row_num in range(2, ws.max_row + 1):
        ...
        for col_name in ["approved_by", "approved_at", "approved_fields", "approved_revision"]:
            if col_name in idx:
                ws.cell(row_num, idx[col_name]).value = ""
        ws.cell(row_num, idx["issues"]).value = (
            "R3: image observation and alt rewritten from inspected contact sheet; current admin alt and image URL matched from products_export_1.csv where possible."
        )
        revised_images += 1
    ...
    for row_num in range(2, ws.max_row + 1):
        ...
        ws.cell(row_num, idx["mapping_reason"]).value = "R3 separates baseball pages by visible motif, wording, audience and proof level for customization."
    ...
    for row_num in range(2, ws.max_row + 1):
        ...
        ws.cell(row_num, idx["factual_conflicts"]).value = "Sang QA issues addressed in r3; requires independent QA recheck."
    ...
    for metric, value, definition in [
        ("qa_batch_009_r3_revision", "r3", "Separate 10-product revision after Sang QA; source workbook was not modified."),
        ("qa_batch_009_r3_admin_export", str(ADMIN_EXPORT.relative_to(ROOT)), f"Admin CSV baseline used; sha256:{admin_hash}."),
        ("qa_batch_009_r3_scope", "inventory positions 81-90", "No products outside qa_batch_009 were revised."),
        ("qa_batch_009_r3_revised_products", str(revised_products), "SEO_Products rows revised and kept NEEDS_REVIEW."),
        ("qa_batch_009_r3_revised_images", str(revised_images), "Image_Audit rows revised and kept NEEDS_REVIEW."),
        ("qa_batch_009_r3_admin_alt_matches", str(admin_alt_matches), "Image rows with current admin alt matched by image URL from Shopify CSV."),
    ]:
        ws.append([
