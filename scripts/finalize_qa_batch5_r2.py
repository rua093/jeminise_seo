...
expected_keys = [r["product_key"] for r in inventory if 41 <= int(r["inventory_position"]) <= 50]
...
tests = {
    ...
    "exact_product_keys_41_50": [x["product_key"] for x in products] == expected_keys,
    "product_count_10": len(products) == 10,
    "image_count_72": len(images) == 72,
    "criteria_count_110": len(criteria) == 110,
    "unique_product_keys": len({x["product_key"] for x in products}) == 10,
    "unique_image_keys": len({x["qa_image_key"] for x in images}) == 72,
...
manifest.update({
    ...
    "xlsx_sha256":sha(OUT / "SEO_QA_qa_batch_005_r2.xlsx"),
