...
BATCHES = {
    "010": {
        "qa_run": "20260907_181000",
        "source": "SEO_Product_Optimization_through_batch_010.xlsx",
        "positions": range(91, 101),
        "expected_images": 73,
    },
    "011": {
        "qa_run": "20260907_181100",
        "source": "SEO_Product_Optimization_through_batch_011.xlsx",
        "positions": range(101, 111),
        "expected_images": 77,
    },
    "012": {
        "qa_run": "20260907_181200",
        "source": "SEO_Product_Optimization_through_batch_012.xlsx",
        "positions": range(111, 121),
        "expected_images": 72,
    },
    "013": {
        "qa_run": "20260907_182000",
        "source": "SEO_Product_Optimization_through_batch_013.xlsx",
        "positions": range(121, 131),
        "expected_images": 62,
    },
}
...
def customizer_problem(product: dict, audit: dict | None) -> tuple[str | None, str]:
    ...
    missing = []
    if flags["name"] and int(audit.get("name_mentions") or 0) == 0:
        missing.append("name")
...
def criterion_rows(product: dict, issue_ids: list[str], image_avg: float, live_row: dict | None, custom_sev: str | None) -> list[dict]:
    ...
    reasons = {
        ...
        "T1": "Title/H1 are product-specific and tied to visible motif/product type.",
...
def build_batch(batch_id: str, cfg: dict) -> dict:
    ...
    for product in products:
        ...
        if any(term in cluster_terms for term in ("soccer", "semi truck", "wolf", "christian", "cross", "football")):
            issue_refs.append(
                add_issue(
                    qa_issues,
                    pk,
