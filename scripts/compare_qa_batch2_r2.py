...
REVISION = ROOT / "resutls" / "jeminise.com" / "20260906_234129" / "revisions" / "qa_batch_002_r2" / "SEO_Product_Optimization_qa_batch_002_r2.xlsx"
OUTPUT = RUN / "qa" / "20260907_123726" / "revision_diff.json"
...
def main() -> None:
    ...
    selected = inventory[10:20]
    keys = {x["product_key"] for x in selected}
    handles = {x["Handle"] for x in selected}
    urls = {x["product_url"] for x in selected}
    specs = {
    ...
    for sheet, (include, identify) in specs.items():
        ...
        for rid in sorted(set(before) | set(after)):
            ...
            fields = [{"field": field, "before": old.get(field), "after": new.get(field)} for field in sorted(set(old) | set(new)) if old.get(field) != new.get(field)]
            if fields:
                changes.append({"row_id": rid, "changes": fields})
