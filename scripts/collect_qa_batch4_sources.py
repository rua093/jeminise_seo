...
module.QA_RUN_ID = "20260907_095643"
...
def main():
    with module.INVENTORY.open(newline="", encoding="utf-8-sig") as handle:
        batch = list(csv.DictReader(handle))[30:40]
    ...
    for row in batch:
        ...
        checkpoint = {
            "rubric_version": "1.0",
            "qa_run_id": module.QA_RUN_ID,
            "source_workbook": str(module.WORKBOOK),
            "batch_id": "qa_batch_004",
            "batch_product_keys": [item["product_key"] for item in batch],
            "current_product_key": row["product_key"],
            "current_stage": "LIVE_SOURCE_COLLECTED",
            "completed_image_keys": [],
            "awaiting_confirmation": False,
        }
        (module.QA_DIR / "qa_progress.json").write_text(
