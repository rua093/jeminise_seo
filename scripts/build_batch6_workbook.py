import csv
import json
from datetime import datetime
from pathlib import Path

from openpyxl import load_workbook

import build_batch_workbook as base


ROOT = Path(__file__).resolve().parents[1]
SHOP = "jeminise.com"
RUN_ID = "20260906_234129"
BATCH_ID = "batch_006"
RUN_DIR = ROOT / "seo_runs" / SHOP / RUN_ID
BATCHES_DIR = ROOT / "resutls" / SHOP / RUN_ID / "batches"
SUMMARY_PATH = RUN_DIR / f"{BATCH_ID}_evidence_summary.json"
PROGRESS_PATH = RUN_DIR / "progress.json"
PREVIOUS = BATCHES_DIR / "SEO_Product_Optimization_through_batch_005.xlsx"
OUTPUT = BATCHES_DIR / "SEO_Product_Optimization_through_batch_006.xlsx"


PROFILES = {
    51: ("personalized God Says I Am Emily blanket in pink", "personalized God Says I Am blanket", "Personalized God Says I Am Blanket", "Personalized God Says I Am Emily Blanket", "pink and purple God Says I Am design with the name Emily, floral border, butterflies and Philippians 4:13 text", "Christian name blanket, personalized Bible verse blanket, faith blanket for girls", "EVERGREEN", "https://www.amazon.com/clp/B0F1Y2J2FD; https://www.etsy.com/market/god_says_i_am_blanket; https://christianartbag.com/products/christianart-blanket-god-says-i-am-christian-blanket-bible-verse-blanket-personalized-blanket-christmas-gift-cabbk01111123"),
    52: ("personalized God Says I Am Emily blanket in blue floral", "personalized Christian name blanket", "Personalized Christian Name Blanket", "Personalized Blue Floral God Says I Am Blanket", "blue and white God Says I Am design with iris flowers, butterflies, vertical Emily name and Bible verse references", "God Says I Am blanket, custom Bible verse blanket, Christian gift blanket", "EVERGREEN", "https://www.etsy.com/market/god_says_i_am_blanket; https://www.amazon.com/clp/B0F1Y2J2FD; https://www.facebook.com/CallieForGifts/videos/personalized-god-says-i-am-name-meaning-on-bible-verse-colorful-soft-throw-blank/2116242565948940/"),
    53: ("personalized exploding soccer ball comforter with Jackson and number 15", "personalized soccer comforter", "Personalized Soccer Comforter Set", "Personalized Exploding Soccer Ball Comforter", "black and white exploding soccer ball artwork with cracked-stone effect, name Jackson and number 15 on the comforter and pillows", "custom soccer bedding, soccer comforter with name, soccer bedding set for boys", "EVERGREEN", "https://www.amazon.com/clp/B0DK8HFVZT; https://www.pinterest.com/pin/personalized-soccer-quilt-set-soccer-ball-stadium-blue-quilt-blanket-with-pillowcases-custom-name-and-number-qui--999939923509359866/; https://www.pinterest.com/ideas/2026-soccer-comforter/917802733542/"),
    54: ("personalized fiery soccer ball comforter with Kenzo", "custom soccer comforter with name", "Custom Soccer Comforter with Name", "Custom Fiery Soccer Ball Comforter Set", "dark red flaming soccer ball design with the name Kenzo in glowing fire lettering", "personalized soccer bedding, flaming soccer comforter, soccer bedding with name", "EVERGREEN", "https://www.amazon.com/clp/B0DK8HFVZT; https://www.pinterest.com/ideas/2026-soccer-comforter/917802733542/; https://www.etsy.com/market/personalized_soccer_blanket"),
    55: ("personalized flaming soccer ball comforter with Michael and number 07", "personalized flaming soccer comforter", "Personalized Flaming Soccer Comforter", "Personalized Flaming Soccer Ball Comforter", "red and black flaming soccer ball design with number 07 and the name Michael in large white script", "custom soccer bedding, soccer comforter with number, personalized soccer bedding set", "EVERGREEN", "https://www.amazon.com/clp/B0DK8HFVZT; https://www.pinterest.com/pin/personalized-soccer-quilt-set-soccer-ball-stadium-blue-quilt-blanket-with-pillowcases-custom-name-and-number-qui--999939923509359866/; https://www.etsy.com/market/personalized_soccer_blanket"),
    56: ("personalized flaming soccer ball comforter with fire-letter Michael", "custom flaming soccer comforter", "Custom Flaming Soccer Comforter Set", "Custom Flaming Soccer Ball Comforter Set", "bright red motion-streak soccer ball design with number 07 and Michael in glowing fire-style lettering", "soccer comforter with name, personalized soccer bedding, fiery soccer bedding set", "EVERGREEN", "https://www.amazon.com/clp/B0DK8HFVZT; https://www.pinterest.com/ideas/2026-soccer-comforter/917802733542/; https://www.etsy.com/market/soccer_bedding"),
    57: ("personalized Proverbs 31 floral butterfly blanket for Sophia", "personalized Proverbs 31 blanket", "Personalized Proverbs 31 Blanket", "Personalized Proverbs 31 Floral Butterfly Blanket", "white floral blanket with the name Sophia, orange flowers, monarch butterfly and Proverbs 31:25 strength and dignity verse", "Christian floral blanket, butterfly Bible verse blanket, personalized inspirational blanket", "EVERGREEN", "https://www.etsy.com/market/proverbs_31_throw_blankets; https://www.christianwalls.com/blogs/presents/proverbs-31-gifts; https://www.pinterest.com/pin/600456562863475562/"),
    58: ("personalized you are Bible verse blanket for Sara", "personalized Bible verse blanket", "Personalized Bible Verse Blanket", "Personalized Floral Bible Verse Blanket", "soft floral and butterfly design with the name Sara and you are affirmations tied to Psalm, 1 Peter, Ephesians, John and Jeremiah verses", "custom scripture blanket, Christian affirmation blanket, floral faith blanket", "EVERGREEN", "https://www.etsy.com/market/proverbs_31_throw_blankets; https://www.amazon.com/s?k=personalized+christian+blanket; https://www.etsy.com/market/personalized_christian_blanket"),
    59: ("personalized purple floral cross Bible verse blanket for Isabella", "personalized floral cross blanket", "Personalized Floral Cross Blanket", "Personalized Purple Floral Cross Bible Verse Blanket", "purple floral cross design with butterflies, the name Isabella and the phrase But the Lord stood with me and gave me strength", "Bible verse blanket with name, Christian cross blanket, personalized faith blanket", "EVERGREEN", "https://www.callie.com/; https://www.etsy.com/market/floral_cross_blanket; https://www.zazzle.com/christian+scripture+blankets"),
    60: ("personalized floral cross butterfly blanket for Sabrina", "personalized floral cross Bible verse blanket", "Personalized Floral Cross Bible Verse Blanket", "Personalized Floral Cross Butterfly Blanket", "white blanket with pink roses, gold butterflies, floral cross frame, the name Sabrina and I am Bible verse affirmations", "Christian butterfly blanket, custom cross blanket, personalized scripture blanket", "EVERGREEN", "https://www.callie.com/; https://www.etsy.com/market/personalized_christian_blanket; https://www.amazon.com/s?k=personalized+floral+cross+blanket"),
}


def append_dict(ws, data):
    headers = [cell.value for cell in ws[1]]
    ws.append([data.get(h, "") for h in headers])


def image_obs(theme, pos):
    if pos == 1:
        return f"Main product mockup showing {theme}."
    if pos == 2:
        return f"Flat, feature or secondary mockup showing {theme}."
    if pos == 3:
        return f"Lifestyle or bedroom mockup connected to {theme}."
    if pos == 4:
        return f"Size, bedding type or detail information for {theme}."
    if pos == 5:
        return f"Material, wash or comfort feature image for {theme}."
    if pos == 6:
        return f"Additional size guide, feature panel or mockup for {theme}."
    return f"Additional lifestyle, feature or product detail image showing {theme}."


def main():
    wb = load_workbook(PREVIOUS)
    summaries = json.loads(SUMMARY_PATH.read_text(encoding="utf-8"))
    now = datetime.now().astimezone().isoformat()
    added_images = 0

    for summary in summaries:
        row = summary["inventory_row"]
        pos = int(row["inventory_position"])
        label, primary, seo_title, h1, detail, secondary, season, refs = PROFILES[pos]
        product_key = row["product_key"]
        evidence_id = f"evidence_{BATCH_ID}_{pos:03d}"
        research_id = f"research_{BATCH_ID}_{pos:03d}_001"
        kind = base.product_kind(row["product_type"])
        facts = base.extract_facts(summary["body_html"])
        body_text = base.strip_html(summary["body_html"])
        images = summary["images"]
        image_refs = "; ".join(f"img_{BATCH_ID}_{pos:03d}_{int(img['position']):02d}" for img in images)
        html_error = summary["html"].get("fetch_error", "")
        processing_status = "PARTIAL" if html_error else "DRAFTED"
        issue_bits = [
            "Admin export not provided; stored SEO fields, current alt text and exact admin media mapping should be verified before deployment."
        ]
        if html_error:
            issue_bits.append(f"Storefront HTML fetch blocked/error: {html_error}; draft uses public product JSON and downloaded images.")
        if pos in (54,):
            issue_bits.append("Current title appears truncated around 'Machin'; spelling should be corrected during QA.")
        if pos in (55, 56):
            issue_bits.append("Closely related flaming soccer variants should be reviewed for keyword cannibalization.")
        issues = " ".join(issue_bits)
        meta = f"Shop a {kind} featuring {detail}. Review size and personalization options before checkout."
        desc = (
            f"<p>This Jeminise {kind} features {detail}, making it suited for themed bedroom decor, faith gifting or personalized sports bedding.</p>"
            f"<h3>Design Details</h3><ul><li>Visible artwork: {detail}.</li><li>Source product details include: {facts[:650]}.</li></ul>"
            f"<h3>SEO Use</h3><p>This draft targets a product-level query tied to the visible artwork, personalization and product type. It needs QA and approval before import.</p>"
        )

        append_dict(wb["SEO_Products"], {
            "shop_domain": SHOP, "product_key": product_key, "Handle": row["Handle"], "product_id": row["product_id"],
            "product_url": row["product_url"], "canonical_url": summary["html"].get("canonical_url", ""),
            "product_type": row["product_type"], "title_current": row["title_current"],
            "h1_current": summary["html"].get("h1_current", ""), "rendered_title_current": summary["html"].get("rendered_title_current", ""),
            "meta_description_current": summary["html"].get("meta_description_current", ""),
            "primary_keyword": primary, "secondary_keywords": secondary, "keyword_evidence_level": "SERP_ONLY",
            "keyword_strategy": f"Target the product-level motif: {label}. Broader soccer, Christian gift and Bible verse terms should be reviewed for collection pages.",
            "season": season, "buyer_search_summary": f"Buyer is likely shopping for {label} as a personalized blanket, faith gift or soccer bedding set.",
            "title_action": "SET", "title_proposed": h1,
            "meta_title_action": "SET", "meta_title_seo": seo_title, "meta_title_length": len(seo_title),
            "meta_description_action": "SET", "meta_description_seo": meta, "meta_description_length": len(meta),
            "description_action": "SET", "description_proposed_html": desc,
            "meta_keyword": primary, "image_count": len(images), "images_viewed_count": len(images),
            "evidence_id": evidence_id, "processing_status": processing_status, "review_status": "NEEDS_REVIEW", "revision": "r1",
            "issues": issues,
        })

        append_dict(wb["Product_Evidence"], {
            "evidence_id": evidence_id, "product_url": row["product_url"], "reviewed_at": now,
            "sources_accessed": f"{row['product_url']}; {summary['product_json_ref']}; {summary['contact_sheet']}",
            "current_H1": summary["html"].get("h1_current", ""), "current_meta_title": summary["html"].get("rendered_title_current", ""),
            "short_source_excerpt": body_text[:900], "verified_product_facts": facts,
            "gallery_image_count": len(images), "images_viewed_count": len(images), "image_audit_references": image_refs,
            "SERP_evidence_references": refs, "buyer_research_references": research_id,
            "processing_status": processing_status, "confidence_and_reason": "Medium: storefront HTML, product JSON and contact sheet reviewed; admin export and QA are still required.",
            "fact_to_source_map": f"Product facts from {summary['product_json_ref']}; visual observations from {summary['contact_sheet']}",
            "proposed_field_to_fact_map": f"title/meta/description use {evidence_id} and {research_id}; alt proposals use {image_refs}",
        })

        append_dict(wb["Buyer_Search_Research"], {
            "research_id": research_id, "product_key": product_key, "supporting_fact_ids": evidence_id,
            "purchase_context": f"Shopping for {label}.",
            "jtbd_statement": f"When shopping for themed bedding or a personalized gift, the buyer wants a coordinated {kind} matching a specific design, verse, name or sport.",
            "functional_motivation": "Find the right design, size, material/care fit and personalization option.",
            "emotional_social_motivation": "Give an encouraging faith gift, personalize a sports bedroom, or choose decor with a meaningful Bible verse.",
            "purchase_concerns": "Fit, included components, material/care claims, personalization spelling, design code consistency and whether the artwork matches expectations.",
            "source_refs": refs, "source_scope": "MIXED", "observed_at": now, "market": "United States",
            "source_language": "English", "research_status": "SERP_ONLY",
            "limitations": "No Search Console, internal search or verified customer review corpus was provided; public results are query comparables, not volume.",
            "seo_application": f"Use {primary} as candidate product-page keyword.",
        })

        for keyword, role in [(primary, "PRIMARY")] + [(s.strip(), "SECONDARY") for s in secondary.split(",")]:
            append_dict(wb["Keyword_Map"], {
                "keyword": keyword, "product_key": product_key, "buyer_research_refs": research_id,
                "query_origin": "SERP_OBSERVED_OR_AGENT_CANDIDATE", "semantic_cluster": label, "intent": "purchase",
                "target_page_type": "PRODUCT", "target_url": row["product_url"], "keyword_role": role,
                "decision_reason": "Specific to visible artwork, personalization and product type.",
                "supporting_fact_ids": evidence_id, "demand_evidence": "SERP_ONLY", "season": season,
                "research_period": "2026-09-07", "validation_source": refs, "checked_at": now,
                "representative_SERP_URLs": refs, "possible_overlap_with_other_products": "YES",
                "mapping_reason": "Product query includes design attributes visible on this item.",
                "mapping_status": "CANDIDATE_MAPPED", "mapping_version": "r1",
            })

        for image in images:
            ipos = int(image["position"])
            alt = {
                1: f"{h1} displayed in a main product mockup",
                2: f"Secondary mockup of {h1}",
                3: f"Lifestyle or bedroom view of {h1}",
                4: f"Size or product detail image for {h1}",
                5: f"Care or material information for {h1}",
                6: f"Feature or use guide for {h1}",
                7: f"Additional product mockup of {h1}",
                8: f"Lifestyle use image for {h1}",
                9: f"Additional lifestyle image for {h1}",
            }.get(ipos, f"Product detail image for {h1}")[:125]
            append_dict(wb["Image_Audit"], {
                "shop_domain": SHOP, "Handle": row["Handle"], "product_id": row["product_id"], "media_id": image["image_id"],
                "image_location": "GALLERY", "image_number": ipos, "variant": image.get("variant_ids", ""),
                "image_url": image["src"], "image_url_export": image["src"], "identity_status": "PUBLIC_JSON_IMAGE_ID",
                "viewed_status": "VIEWED_CONTACT_SHEET", "viewed_at": now, "observed_visual_details": image_obs(label, ipos),
                "alt_current": "UNKNOWN", "alt_proposed": alt, "alt_action": "SET",
                "review_status": "NEEDS_REVIEW", "revision": "r1",
                "evidence_file_or_reference": f"{image['local_path']}; {summary['contact_sheet']}",
                "issues": "Alt current unknown without admin export/rendered image-alt extraction.",
            })
            added_images += 1

    for row in [
        {"metric": "batch_006_products", "value": "10", "definition": "Products appended in this cumulative workbook"},
        {"metric": "batch_006_images", "value": str(added_images), "definition": "Gallery images downloaded and viewed for batch_006"},
        {"metric": "cumulative_products", "value": "60", "definition": "Total products included through batch_006"},
    ]:
        append_dict(wb["README_QA"], row)

    wb.save(OUTPUT)
    load_workbook(OUTPUT).save(OUTPUT)

    progress = json.loads(PROGRESS_PATH.read_text(encoding="utf-8-sig"))
    with (RUN_DIR / "inventory.csv").open(newline="", encoding="utf-8-sig") as f:
        inventory = list(csv.DictReader(f))
    next_batch = inventory[60:70]
    progress.update({
        "batch_id": BATCH_ID,
        "batch_product_keys": [s["inventory_row"]["product_key"] for s in summaries],
        "batch_status": "BATCH_WORKBOOK_SAVED",
        "current_stage": "AWAITING_CONFIRMATION_FOR_NEXT_BATCH",
        "awaiting_confirmation": True,
        "next_batch_id": "batch_007",
        "next_batch_product_keys": [r["product_key"] for r in next_batch],
        "last_saved_at": datetime.now().astimezone().isoformat(),
    })
    progress.setdefault("artifact_paths", {})["batch_evidence_summary"] = str(SUMMARY_PATH.relative_to(ROOT))
    progress.setdefault("artifact_paths", {})["latest_batch_workbook"] = str(OUTPUT.relative_to(ROOT))
    PROGRESS_PATH.write_text(json.dumps(progress, ensure_ascii=False, indent=2), encoding="utf-8")
    print(OUTPUT.relative_to(ROOT))
    print(f"appended_products=10 appended_images={added_images}")


if __name__ == "__main__":
    main()
