...
SOURCE = ROOT / "resutls" / "jeminise.com" / "20260906_234129" / "revisions" / "qa_batch_007_r2" / "SEO_Product_Optimization_qa_batch_007_r2.xlsx"
...
expected_keys = [r["product_key"] for r in inventory if 61 <= int(r["inventory_position"]) <= 70]

products, images = dataset["QA_Products"], dataset["QA_Images"]
issues, criteria = dataset["QA_Issues"], dataset["QA_Criteria"]
...
tests = {
    ...
    "exact_product_keys_61_70": [x["product_key"] for x in products] == expected_keys,
    "product_count_10": len(products) == 10, "image_count_60": len(images) == 60,
    "criteria_count_110": len(criteria) == 110,
    "unique_product_keys": len({x["product_key"] for x in products}) == 10,
    "unique_image_keys": len({x["qa_image_key"] for x in images}) == 60,
...
manifest.update({"status": "COMPLETE", "source_sha256_at_handoff": sha(SOURCE), "source_snapshot_sha256": sha(SNAPSHOT),
    "xlsx_sha256": sha(OUT / "SEO_QA_qa_batch_007_r2.xlsx"), "validation_passed": True,
