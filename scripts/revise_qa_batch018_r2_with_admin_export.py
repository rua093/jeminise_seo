...
BATCH_ID = "qa_batch_018_r2"
QA_RUN_ID = "20260907_213100"

SOURCE = ROOT / "resutls" / SHOP / RUN_ID / "batches" / "SEO_Product_Optimization_through_batch_018.xlsx"
...
OUTPUT = RESULT_DIR / "SEO_Product_Optimization_qa_batch_018_r2.xlsx"
REPORT = RESULT_DIR / "SEO_Product_Optimization_qa_batch_018_r2.md"
...
PRODUCT_UPDATES = {
    ...
    176: {
        ...
        "intent_role": "Baseball bedding page supported by required name and number customizer fields and separated by vertical flag-name layout.",
        "customizer": "Required fields captured: Customize Your Name and Customize Your Number, plus a product-type confirmation control.",
        "custom_supported": True,
    },
    177: {
        ...
        "intent_role": "Baseball bedding page supported by required name and number fields and separated by black glove-and-bat layout.",
        "customizer": "Required fields captured: Customize Your Name and Customize Your Number, plus a product-type confirmation control.",
        "custom_supported": True,
    },
    178: {
    ...
    179: {
        ...
        "intent_role": "Baseball bedding page supported by required name and number fields and separated by home quote typography.",
        "customizer": "Required fields captured: Customize Your Name and Customize Your Number, plus a product-type confirmation control.",
        "custom_supported": True,
    },
    180: {
...
def main():
    ...
    for row_num in range(2, ws.max_row + 1):
        ...
        ws.cell(row_num, idx["issues"]).value = (
            f"R2 after Sang QA batch 018: used products_export_1.csv sha256:{admin_hash}; "
            "removed internal workflow copy, filled publishable title values, corrected unsupported personalization/file-submission claims, "
    ...
    for row_num in range(2, ws.max_row + 1):
        ...
        ws.cell(row_num, idx["jtbd_statement"]).value = (
            ...
            "and whether the relevant custom fields are actually available."
        )
        ws.cell(row_num, idx["functional_motivation"]).value = (
            "Confirm product type, visible motif, size or fit panels, supported custom fields and image accuracy before purchase."
        )
        ...
        ws.cell(row_num, idx["purchase_concerns"]).value = (
            "Avoid unsupported name, number, image-submission, cultural-origin or material claims; confirm exact size and product type."
        )
    ...
    for row_num in range(2, ws.max_row + 1):
        ...
        ws.cell(row_num, idx["verified_product_facts"]).value = (
            ...
            f"custom_supported={item['custom_supported']}; customizer_note={item['customizer']}"
        )
    ...
    for metric, value, definition in [
        ("qa_batch_018_r2_revision", "r2", "Separate 10-product revision after Sang QA; source workbook was not modified."),
        ("qa_batch_018_r2_admin_export", str(ADMIN_EXPORT.relative_to(ROOT)), f"Admin CSV baseline used; sha256:{admin_hash}."),
        ("qa_batch_018_r2_scope", "inventory positions 171-180", "No products outside qa_batch_018 were revised."),
        ("qa_batch_018_r2_revised_products", str(revised_products), "SEO_Products rows revised and kept NEEDS_REVIEW."),
        ("qa_batch_018_r2_revised_images", str(revised_images), "Image_Audit rows revised and kept NEEDS_REVIEW."),
        ("qa_batch_018_r2_admin_alt_matches", str(admin_alt_matches), "Image rows with current admin alt matched by image URL from Shopify CSV."),
    ]:
        ws.append([
    ...
    REPORT.write_text(
        "\n".join(
            [
                ...
                "- Sửa trọng yếu: bỏ nội dung nội bộ, bổ sung title/H1 publishable, sửa claim cá nhân hóa theo customizer audit, tách intent cho Halloween/music/Tree of Life/baseball theo motif nhìn thấy và viết lại mô tả/alt theo contact sheets.",
