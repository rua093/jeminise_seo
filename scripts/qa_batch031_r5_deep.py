...
QA_RUN_ID = "20260909_141012"
BATCH_ID = "qa_batch_031_r5"
SOURCE = ROOT / "resutls" / SHOP / RUN_ID / "revisions" / "qa_batch_031_r5" / "SEO_Product_Optimization_qa_batch_031_r5.xlsx"
...
PRODUCT_310 = "evidence_batch_031_310"

SERP_NOTES = {
    "cactus": "Public SERP showed comparable cactus/southwest floral quilt bedding pages from Amazon-style store pages, Lush Decor, Wayfair and Walmart; no first-party volume.",
    "viking": "Public SERP showed comparable Yggdrasil/Celtic/Tree of Life quilt bedding pages; no first-party volume.",
    "blanket": "Public SERP showed comparable personalized reading/floral blanket and basketball blanket pages; no first-party volume.",
    "deer": "Public SERP showed comparable deer quilt and woodland couple bedding pages, including public shopping/listing pages; no first-party volume.",
}
...
def product_group(handle):
    if "cactus" in handle:
        return "cactus"
    if "viking" in handle or "yggdrasil" in handle:
        return "viking"
    if "blanket" in handle or "basketball" in handle:
        return "blanket"
    return "deer"
...
def main():
    ...
    for raw in ws.iter_rows(min_row=2, values_only=True):
        ...
        if str(row.get("evidence_id") or "").startswith("evidence_batch_031_"):
            products.append(row)
    ...
    for p in products:
        ...
        proposed_fields = " ".join(str(p.get(c) or "") for c in ["primary_keyword", "secondary_keywords", "title_proposed", "meta_title_seo", "meta_description_seo", "description_proposed_html", "meta_keyword"])
        meta = p.get("meta_description_seo") or ""
        body = html_text(p.get("description_proposed_html"))
        is_310 = p.get("evidence_id") == PRODUCT_310
        proposed_type_conflict = is_310 and "comforter" in proposed_fields.lower()
        ...
        if proposed_type_conflict:
            ...
            issues.append({
                ...
                "submitted_value": "Proposed fields still contain comforter wording.",
                "source_observation": f"Admin Type={first.get('Type')}; live JSON type={l['json_type']}; options={l['options']}; evidence supports Quilt/Quilt Set.",
                "reason": "Product 310 proposed wording still conflicts with stronger product type evidence.",
                "recommended_fix": "Remove comforter wording from proposed public fields and image alt text.",
                "supporting_evidence": evrefs,
                "recheck_condition": "No comforter wording remains in proposed public fields or image alt text.",
            })
            issue_no += 1
        ...
        if p.get("revision") != "r5":
            ...
            issues.append({
                ...
                "source_observation": "Source workbook path is qa_batch_031_r5 but row-level revision remains from an older row.",
                "reason": "Traceability metadata is inconsistent. It does not change copy correctness but weakens audit clarity.",
                "recommended_fix": "Update row-level revision metadata when a product is substantively revised.",
                "supporting_evidence": str(SOURCE.relative_to(ROOT)),
                "recheck_condition": "Row revision matches product's latest substantive revision.",
            })
            issue_no += 1
            issue_refs.append(iid)
            minor += 1
        ...
        for img in imgs:
            ...
            img_conflict = is_310 and "comforter" in (obs + " " + alt).lower()
            ...
            qa_images.append({
                ...
                "issue_refs": "TYPE_CONFLICT" if img_conflict else "",
        ...
        e1_rating = "FULL" if p.get("revision") == "r5" else "PARTIAL"
        criteria = [
            criterion(product_key, "P1", "FULL", "Product type, artwork, color and design details match evidence; product 310 has been corrected to Quilt/Quilt Set in proposed fields.", evrefs),
            criterion(product_key, "P2", "FULL", "Options and personalization details are grounded in live/admin/workbook evidence; no unsupported shipping or material promise is added.", evrefs),
            criterion(product_key, "K1", "FULL", "Long-tail target fits the visible design and product-page intent.", f"{evrefs}; {SERP_NOTES[group]}"),
            ...
            criterion(product_key, "K3", "PARTIAL", "Evidence remains SERP_ONLY; no first-party or paid keyword demand source supplied.", SERP_NOTES[group], issue_refs[0]),
            criterion(product_key, "T1", "FULL", "SEO title is natural, specific and aligned with product type.", evrefs),
            criterion(product_key, "T2", "FULL", "Product title proposal is useful and consistent with product evidence.", evrefs),
            criterion(product_key, "D1", "PARTIAL" if meta_cut else "FULL", "Meta description is complete, specific and not cut mid-word or mid-idea." if not meta_cut else "Meta description may be cut or end abruptly.", evrefs),
            criterion(product_key, "D2", "PARTIAL" if template_leak else "FULL", "Body HTML is complete, design-specific and free of internal/template wording." if not template_leak else "Body contains internal/template wording.", evrefs),
            {"product_key": product_key, "criterion_id": "I1", "weight": 20, "assessment": "DERIVED", "rating": "FULL" if image_avg == 100 else "PARTIAL", "earned_points": i1_points, "assessed_weight": 20, "reason": "All images are present; observations and alt text are specific, truthful and natural." if image_avg == 100 else "All images are present but at least one image alt/observation has a product-type wording issue.", "evidence_refs": "Image_Audit; contact sheets; live JSON image count; admin export image count", "issue_refs": ""},
            criterion(product_key, "E1", e1_rating, "Source can be traced; r5 row metadata matches the revised product." if e1_rating == "FULL" else "Source can be traced, but row revision remains from an older substantive revision.", evrefs, "" if e1_rating == "FULL" else issue_refs[-1]),
        ]
        qa_criteria.extend(criteria)
        score = round(sum(c["earned_points"] for c in criteria), 1)
        has_major = any(i for i in issues if i["product_key"] == product_key and i["severity"] == "MAJOR")
        status = "QA_REVISE" if has_major else ("QA_PASS" if score >= 85 else "QA_FAIL")
        qa_products.append({
            "product_key": product_key,
            ...
            "image_coverage": "100%",
            "critical_count": 0,
            "major_count": 1 if has_major else 0,
            "minor_count": minor,
    ...
    xlsx = OUT_DIR / "SEO_QA_qa_batch_031_r5.xlsx"
    md = OUT_DIR / "SEO_QA_qa_batch_031_r5.md"
    ...
    validation = {
        ...
        "passed": len(qa_products) == 10 and len(qa_images) == 71 and len(qa_criteria) == 110,
    }
    for name, obj in [
    ...
    md.write_text(f"""# SEO QA qa_batch_031_r5
