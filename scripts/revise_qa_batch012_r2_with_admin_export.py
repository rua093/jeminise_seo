...
BATCH_ID = "qa_batch_012_r2"
QA_RUN_ID = "20260907_181200"

SOURCE = ROOT / "resutls" / SHOP / RUN_ID / "batches" / "SEO_Product_Optimization_through_batch_012.xlsx"
...
OUTPUT = RESULT_DIR / "SEO_Product_Optimization_qa_batch_012_r2.xlsx"
REPORT = RESULT_DIR / "SEO_Product_Optimization_qa_batch_012_r2.md"
...
def main():
    ...
    for row_num in range(2, ws.max_row + 1):
        ...
        ws.cell(row_num, idx["issues"]).value = (
            f"R2 after Sang QA batch 012: used products_export_1.csv sha256:{admin_hash}; "
    ...
    for row_num in range(2, ws.max_row + 1):
        ...
        ws.cell(row_num, idx["mapping_reason"]).value = (
            "R2 separates nearby pages by visible motif, sample wording, color palette and proof level for customization."
        )
    ...
    for metric, value, definition in [
        ("qa_batch_012_r2_revision", "r2", "Separate 10-product revision after Sang QA; source workbook was not modified."),
        ("qa_batch_012_r2_admin_export", str(ADMIN_EXPORT.relative_to(ROOT)), f"Admin CSV baseline used; sha256:{admin_hash}."),
        ("qa_batch_012_r2_scope", "inventory positions 111-120", "No products outside qa_batch_012 were revised."),
        ("qa_batch_012_r2_revised_products", str(revised_products), "SEO_Products rows revised and kept NEEDS_REVIEW."),
        ("qa_batch_012_r2_revised_images", str(revised_images), "Image_Audit rows revised and kept NEEDS_REVIEW."),
        ("qa_batch_012_r2_admin_alt_matches", str(admin_alt_matches), "Image rows with current admin alt matched by image URL from Shopify CSV."),
    ]:
        ws.append([
